...
export const updatePaymentType = async (data: TParamsEditPaymentType) => {
  const { id, ...rests } = data
  try {
    const res = await instanceAxios.put(`${API_ENDPOINT.SETTINGS.PAYMENT_TYPE.INDEX}/${id}`, rests)
    return res.data
  } catch (error: any) {
    return error?.response?.data
  }
...
export const getDetailsPaymentType = async (id: string) => {
  try {
    const res = await instanceAxios.get(`${API_ENDPOINT.SETTINGS.PAYMENT_TYPE.INDEX}/${id}`)
    return res.data
  } catch (error: any) {
    return error?.response?.data
  }
...
export const deletePaymentType = async (id: string) => {
  try {
    const res = await instanceAxios.delete(`${API_ENDPOINT.SETTINGS.PAYMENT_TYPE.INDEX}/${id}`)
    return res.data
  } catch (error: any) {
    console.error('Error during login:', error)
    return error?.response?.data
